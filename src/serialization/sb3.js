...
const serialize = function (runtime, targetId) {
    ...
    // Prevent duplicate stage objects - ensure only one stage exists
    const filteredTargets = [];
    let stageFound = false;
    for (const target of originalTargetsToSerialize) {
        if (target.isStage) {
            if (!stageFound) {
                filteredTargets.push(target);
                stageFound = true;
            }
            // Skip any additional stage objects
        } else {
            filteredTargets.push(target);
        }
    }

    const layerOrdering = getSimplifiedLayerOrdering(filteredTargets);

    const flattenedOriginalTargets = filteredTargets.map((t) => t.toJSON());
...
const deserialize = function (json, runtime, zip, isSingleSprite) {
    ...
    // Prevent deserialization of corrupted data with duplicate stage objects
    if (!isSingleSprite) {
        const filteredTargetObjects = [];
        let stageFound = false;
        for (const target of targetObjects) {
            if (target.isStage) {
                if (!stageFound) {
                    filteredTargetObjects.push(target);
                    stageFound = true;
                }
                // Skip any additional stage objects
            } else {
        ...
        targetObjects = filteredTargetObjects;
    }
