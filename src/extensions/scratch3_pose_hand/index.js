...
class Scratch3PoseNetBlocks {
    ...
    get VIDEO_STATE_INFO() {
        return [
            ...
            },
            {
                ...
                value: HandVideoState.ON_FLIPPED,
            },
        ];
    }
    ...
    getInfo() {
        ...
        if (this.firstInstall) {
            this.globalVideoState = HandVideoState.ON;
                ...
                "---",
                {
                    ...
                    arguments: {
                        VIDEO_STATE: {
                            type: ArgumentType.NUMBER,
                            menu: "VIDEO_STATE",
                            defaultValue: HandVideoState.OFF,
                        },
                    },
                },
                {
    ...
    goToHandPart(args, util) {
        if (this.handPoseState && this.handPoseState.length > 0) {
            const [x, y, z] =
                this.handPoseState[0].annotations[args.HAND_PART][
                    args.HAND_SUB_PART
                ];
            const { x: scratchX, y: scratchY } = this.tfCoordsToScratch({
                x,
                y,
                z,
            });
    ...
    handPosePositionX(args, util) {
        return this.handPoseState.length > 0
            ? this.tfCoordsToScratch({
                  x: this.handPoseState[0].annotations[args.HAND_PART][
                      args.HAND_SUB_PART
                  ][0],
              }).x
            : 0;
    ...
    handPosePositionY(args, util) {
        return this.handPoseState.length > 0
            ? this.tfCoordsToScratch({
                  y: this.handPoseState[0].annotations[args.HAND_PART][
                      args.HAND_SUB_PART
                  ][1],
              }).y
            : 0;
    ...
    videoToggle(args) {
        ...
        if (state === HandVideoState.OFF) {
            this.runtime.ioDevices.video.disableVideo();
        } else {
            this.runtime.ioDevices.video.enableVideo();
            // Mirror if state is ON. Do not mirror if state is ON_FLIPPED.
            this.runtime.ioDevices.video.mirror = state === HandVideoState.ON;
        }
    }
